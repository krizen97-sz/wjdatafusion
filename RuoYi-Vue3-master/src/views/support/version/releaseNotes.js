export const releaseNotes = [
  {
    version: 'v3.12.0',
    submitTime: '2026-08-22 13:10:24',
    level: 'minor',
    levelLabel: '小版本',
    tagType: 'primary',
    title: '巡检看板本地图表系统升级',
    summary: '借鉴数据契约与快读型图表方法，在不引入外部模板和依赖的前提下，重构巡检总览与展开看板，并沉淀可测试、可复用的本地 ECharts 图表能力。',
    changes: [
      '巡检总览从指标陈列改为结论先行，直接提示本周是否稳定、异常子项数量和优先查看方向。',
      '近7天趋势使用巡检总量柱与异常折线双编码，标注异常峰值日期，避免相近折线难以比较。',
      '今日结果构成由环形饼图改为直接标数的横向堆叠条，正常、异常和未检测数量一眼可读。',
      '工具健康度按最差优先排序，增加90%关注线和红/橙/绿状态分层，快速定位薄弱工具。',
      '异常分布由饼图改为异常TopN横向排名，直接显示问题集中步骤与异常次数。',
      '当月日历升级为本地ECharts双序列热力图，同时表达每日巡检次数强度和异常状态。',
      '新增inspectionChartSystem.js，统一图表颜色令牌、空态、tooltip、ARIA、动画和option构建器，并补充模块内数据契约文档。',
      '全部资源随前端构建本地打包，不使用Lieflat模板代码、Chart.js、Google Fonts、CDN或外部网络资源。'
    ],
    scope: ['自动化巡检', '巡检总览', '巡检看板', 'ECharts', '趋势图', '异常TopN', '日历热力', '本地化', '设计规范', '版本记录'],
    database: '无数据库结构或业务数据修改；继续复用现有巡检看板聚合接口。',
    scripts: []
  },
  {
    version: 'v3.11.2',

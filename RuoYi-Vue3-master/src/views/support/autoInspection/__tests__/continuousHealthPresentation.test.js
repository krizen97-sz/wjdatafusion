...
test('monthly summary counts managed sites and excludes unassigned plans from health', () => {
  ...
  assert.equal(summary.warningSiteCount, 0)
...
test('workspace exposes plan mode, daily health and activity tools', () => {
  ...
  assert.ok(panelSource.includes('v-for="site in day.sites"'))
  ...
  assert.ok(panelSource.includes('class="continuous-health-day-list"'))
  assert.ok(panelSource.includes('class="continuous-health-day"'))
  assert.ok(panelSource.includes('v-for="day in pagedRows"'))
  assert.ok(panelSource.includes('<el-collapse-transition>'))
  assert.ok(panelSource.includes(':aria-expanded="expandedDate === day.healthDate"'))
  assert.ok(panelSource.includes('v-show="expandedDate === day.healthDate"'))
  assert.ok(panelSource.includes('@click="toggleDay(day.healthDate)"'))
  assert.ok(panelSource.includes('function todayDateKey()'))
  assert.ok(panelSource.includes('return dates.includes(today) ? today : \'\''))
  assert.ok(panelSource.includes('class="continuous-health-day__date"'))
  ...
  assert.ok(panelSource.includes('<OfficeBuilding />'))
  assert.ok(panelSource.includes('<Monitor />'))
  assert.ok(panelSource.includes('@click="openMonthlyIssue(\'2\')"'))
  assert.ok(panelSource.includes('@click="openMonthlyIssue(\'4\')"'))
  assert.ok(panelSource.includes("emit('day-results', { ...payload, resultStatus })"))
  assert.ok(!panelSource.includes('<el-table-column type="expand"'))
  assert.ok(!panelSource.includes('expandedDates'))
  assert.ok(!panelSource.includes('padding: 8px 12px 14px 52px'))
  assert.ok(workspaceSource.includes('@manage-unassigned="switchConfigTab(\'plan\')"'))
  assert.ok(workspaceSource.includes('resultStatus: route.query.resultStatus'))

...
test('workspace exposes plan mode, daily health and activity tools', () => {
  ...
  assert.ok(!panelSource.includes('class="continuous-health-summary"'))
  assert.ok(!panelSource.includes('纳管现场'))
  assert.ok(!panelSource.includes('纳管主平台'))
  assert.ok(!panelSource.includes('本月异常现场'))
  ...
  assert.ok(panelSource.includes('margin: 8px 12px 14px 20px'))

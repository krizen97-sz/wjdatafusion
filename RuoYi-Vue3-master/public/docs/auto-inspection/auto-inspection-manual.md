...
自动化巡检是平台面向日常运维巡查、系统健康检查和周期性报告归档建设的统一巡检能力。它把过去分散在人工登录、脚本执行、共享文档记录和临时截图里的检查动作，沉淀为“巡检工具、巡检步骤、巡检模板、巡检计划、巡检记录、巡检报告”这一套闭环流程。用户可以按业务系统自由组合模板，再为计划独立配置执行周期以及“逐次记录 / 每日汇总”结果方式，最终形成可查询、可追溯、可按天汇总的巡检结果。
...
`巡检驾驶舱` 用于统一查看逐次执行与每日汇总结论。`巡检总览` 用于查看按日记录、明细和报告。`巡检配置` 用于维护巡检模板和巡检计划。`巡检版本记录` 用于查看自动化巡检模块的独立版本号、关联平台总版本号、修改重点和涉及 SQL。
...
巡检驾驶舱是日常值守的第一入口。它不会把密集采样全部堆到记录列表，而是把逐次执行记录和每日健康汇总为同一天的综合结论。

| 区域 | 说明 |
| --- | --- |
| 今日综合健康 | 按逐次正常次数与每日汇总应执行采样统一计算健康度，并显示健康、需关注、异常或未执行 |
| 近七日综合趋势 | 同时展示每日综合健康度、逐次执行次数和每日汇总完成采样数 |
| 当前计划状态 | 汇总所有启用计划，可按逐次记录和每日汇总比较当前结果 |
| 今日计划健康清单 | 只展示按当前Cron今天需要执行的计划，支持按汇总方式和关键词筛选，并显示今日安排 |
...
| 逐次记录 / 每日汇总 Tabs | 顶部默认Tabs在完整执行记录和每日健康汇总之间切换，刷新后保留当前视图 |
...
3. 需要周期归档时点击“导出周/月报”。
4. 需要看整体趋势时点击“展开看板”。
5. 需要查看按日汇总的计划时切换到“每日健康汇总”，点击日期行的“查看”打开分页执行记录，再展开某一次查看步骤、目标和判定依据。
...
| 结果汇总方式 | 逐次记录为每次执行生成完整记录；每日汇总按日期统计健康度并保留采样明细 |
| 巡检模板 | 选择已经验证过的模板 |
| 任务编码 | 对应平台定时任务编码，用于排查调度 |
| 执行周期 | 页面可视化选择每日、每周、每月或间隔执行 |
| Cron 预览 | 系统自动生成，只读展示 |
| 状态 | 启用后按计划执行，暂停后不再触发 |
| 生效时段 | 每日汇总计划只在配置的业务时段内计入应执行次数 |
| 数据等待 | 每日汇总取数窗口向前延迟，给上游消息和数据库留出落地时间 |
| 健康目标 | 每日汇总健康度的目标百分比 |
...
3. 对关键业务建议设置固定每日巡检时间。
4. 执行周期与汇总方式独立配置；密集采样通常选择间隔执行与每日汇总，SSH递归目录等重型步骤不建议一分钟执行。

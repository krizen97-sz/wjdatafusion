...
| 配置区 | 说明 |
| --- | --- |
| 请求信息 | 配置接口名称、GET/POST、URL 和 HTTPS 证书校验方式 |
| 参数与鉴权 | 支持 Query、Header、Cookie、Bearer Token、Basic Auth、API Key、自定义 Header 和请求体；敏感值保存为密文 |
| 返回条件 | 支持状态码、响应耗时、JSON 数字/字符串/布尔/存在性、列表数量、返回内容和返回 Header |

“HTTPS 证书校验”默认保持严格校验。只有 HTTPS 接口使用单位自建证书、测试时报证书校验失败时，才选择“兼容自建证书”；普通 HTTP 或正式证书不需要调整。

...
| 接口与平台探测 | 接口调用测试 | 模拟 GET/POST 请求，并按状态码、JSON 字段、文本、Header、耗时等多断言判断 |
...
| --- | --- |
| 步骤名称 | 展示在记录和报告中的业务名称 |
| 比较规则 | 海康接口数量和 HTTP 健康使用数值阈值；接口调用测试使用多断言全部通过规则 |
| 告警阈值 | 触发异常的边界值 |
...
| 配置区 | 说明 |
| --- | --- |
| 基础请求 | 配置接口名称、GET/POST、URL 和是否信任内网自签名证书 |
| Header / Cookie / 鉴权 | 支持 Bearer Token、Basic Auth、API Key、Cookie、自定义 Header；敏感值保存为密文 |
| Query / 请求体 | 支持 Query 参数、JSON Body、raw text、`application/x-www-form-urlencoded` |
| 结果断言 | 支持状态码、响应耗时、JSON 数字/字符串/布尔/存在性、数组长度、响应文本和响应 Header |
...
2. 计数接口必须填写结果路径。
3. 健康检测可配置期望状态码。
4. 接口调用测试至少保留一条断言；所有断言通过才算正常，任一断言失败会在目标明细中展示失败原因。
5. 保存步骤前先点击“测试目标”。
6. 接口返回很大时，系统只保存脱敏后的响应预览，避免记录过长和敏感信息泄露。

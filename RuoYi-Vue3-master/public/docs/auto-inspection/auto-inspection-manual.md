...
自动化巡检是平台面向日常运维巡查、系统健康检查和周期性报告归档建设的统一巡检能力。它把过去分散在人工登录、脚本执行、共享文档记录和临时截图里的检查动作，沉淀为“巡检工具、巡检步骤、巡检模板、巡检计划、巡检记录、巡检报告”这一套闭环流程。用户可以在页面中选择 Kafka 消费组指标、MQTT 主题活跃、HTTP 接口调用测试、数据库只读查询、FTP 目录、服务器目录、磁盘、端口和服务状态等巡检工具，按业务系统自由组合成模板，再通过例行计划或高频计划自动执行，最终形成可查询、可追溯、可按天汇总的巡检结果。
...
| 区域 | 说明 |
| --- | --- |
| 今日综合健康 | 按例行正常次数与高频应执行采样统一计算健康度，并显示健康、需关注、异常或未执行 |
| 近七日综合趋势 | 同时展示每日综合健康度、例行执行次数和高频完成采样数 |
| 当前计划状态 | 汇总所有启用计划，可直接比较例行计划和高频计划的当前结果 |
| 计划健康清单 | 支持按模式、计划、模板和标签筛选，异常与需关注计划自动排在前面 |
...
| 例行 / 高频切换 | 例行模式查看完整执行记录，高频模式按天查看健康度和异常日期 |
...
3. 需要周期归档时点击“导出周/月报”。
4. 需要看整体趋势时点击“展开看板”。
5. 需要查看分钟级计划时切换到“高频每日健康”，点击日期行的“查看”打开分页执行记录，再展开某一次查看步骤、目标和判定依据。
...
3. 对关键业务建议设置固定每日巡检时间。
4. 高频链路选择“高频监测”，使用每 1 分钟或每 5 分钟等间隔执行；SSH递归目录等重型步骤不建议一分钟执行。
5. 数值型步骤只选择“固定阈值”或“上次结果”一种判定方式。选择上次结果时直接比较本次值与上次值的差值，首次成功采样建立基线并按正常计入健康度。
...
- “当日结论”右侧的说明按钮用于查看状态含义；长调用结果点击“详情”查看完整内容，大数值可悬浮查看完整值。
...
`v3.16.3` 不涉及数据库修改。若环境尚未升级到 `v3.16.2`，仍需执行 `WDF100.0/sql/support_upgrade_20260829_auto_inspection_daily_history_fix_v3_16_2.sql`；脚本只修正有明确基线证据的旧高频记录。

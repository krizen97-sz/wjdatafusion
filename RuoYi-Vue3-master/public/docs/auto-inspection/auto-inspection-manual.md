...
3. 需要周期归档时点击“导出周/月报”。
4. 需要看整体趋势时点击“展开看板”。
5. 需要查看分钟级计划时切换到“高频每日健康”，点击日期行的“查看当日结果”，即可直接看到采样、步骤、目标和判定依据。
...
- 日期行直接展示当日计划和异常摘要；点击“查看当日结果”后，按时间线直接展示每次采样的步骤、目标、本次值、上次值、变化量、判定公式和异常原因。
...
本版本不修改业务表结构；新增数据库查询检查工具注册脚本：`WDF100.0/sql/support_upgrade_20260819_auto_inspection_etl_workflow_v3_10_0.sql`。

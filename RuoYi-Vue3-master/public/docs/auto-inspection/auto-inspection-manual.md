...
3. 需要周期归档时点击“导出周/月报”。
4. 需要看整体趋势时点击“展开看板”。
5. 需要查看分钟级计划时切换到“高频每日健康”，先选日期，再展开具体计划查看采样明细。
...
| 消息队列检测 | Kafka 主题写入中断检测 | 读取各分区末端 Offset，判断持续多久没有新消息写入 |
| 消息队列检测 | Kafka 消费停滞检测 | 上游有新增但消费组提交 Offset 不推进时告警 |
| 消息队列检测 | MQTT 主题活跃度检测 | 后台持续订阅 Topic，按最后消息时间判断关注、异常和恢复 |
...
2. 需要看链路积压时使用 Kafka 消费积压检测；需要判断断流或消费停止时，分别使用 Kafka 主题写入中断检测、Kafka 消费停滞检测。
...
5. 高频计划中的活性工具需要配置“进入关注、确认异常、恢复确认”三项规则，首次执行会先建立采样基线。
...
- 展开日期后查看每个高频计划，再点击“查看采样”下钻到原始执行记录。

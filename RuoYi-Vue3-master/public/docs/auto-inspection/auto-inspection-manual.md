...
高频每日健康与例行巡检记录使用相同的筛选表单、归属日期块、表格行密度、异常行、结果摘要、统计格和操作列规范，切换页面时不会产生两套列表风格。
...
`v4.0.2` 不涉及数据库修改。若环境尚未升级到 `v3.16.2`，仍需执行 `WDF100.0/sql/support_upgrade_20260829_auto_inspection_daily_history_fix_v3_16_2.sql`；脚本只修正有明确基线证据的旧高频记录。

...
自动化巡检是平台面向日常运维巡查、系统健康检查和周期性报告归档建设的统一巡检能力。它把过去分散在人工登录、脚本执行、共享文档记录和临时截图里的检查动作，沉淀为“巡检工具、巡检步骤、巡检模板、巡检计划、巡检记录、巡检报告”这一套闭环流程。用户可以按业务系统自由组合模板，为计划配置执行周期，并明确计划属于某个现场或主平台，最终形成可查询、可追溯、可按天和业务归属汇总的巡检结果。
...
`巡检驾驶舱` 用于查看现场和主平台当日健康。`巡检总览` 用于按日期、现场和主平台查看健康，并下钻执行明细和报告。`巡检配置` 用于维护巡检模板、巡检计划及其业务归属。`巡检版本记录` 用于查看自动化巡检模块的独立版本号、关联平台总版本号、修改重点和涉及 SQL。
...
巡检驾驶舱是日常值守的第一入口。它不会把密集采样全部堆到记录列表，也不再用一个全局健康度覆盖多个现场，而是分别计算每个现场和主平台当天的健康结论。

| 区域 | 说明 |
| --- | --- |
| 今日纳管范围 | 展示纳管现场、主平台、异常现场、关注现场和待归属计划数量 |
| 近七日执行趋势 | 展示每天应执行、已完成和按计划计算的健康变化 |
| 今日现场状态 | 按现场统计健康、关注、异常和未执行数量 |
| 今日现场与主平台健康 | 现场为第一层、主平台为最深层，计划名称作为计算依据展示在对应行 |
| 待处理问题 | 把逐次目标异常、每日汇总异常、关注状态和采样缺失放到同一清单 |
| 最近逐次执行 | 快速进入单次巡检的步骤和目标详情 |

推荐操作顺序：先查看异常现场，再展开对应主平台；出现异常时进入巡检总览按日期和归属查看执行明细，最后根据具体步骤和目标结果排障。

## 4. 巡检总览

![巡检总览](auto-inspection-manual-assets/01-overview-records.png)

巡检总览打开后优先展示按日期汇总的现场健康清单。展开日期后可查看现场公共计划和主平台健康；逐次执行记录保留在“全部执行记录”抽屉中，作为健康结论的审计证据。

页面主要区域如下：

| 区域 | 说明 |
| --- | --- |
| 月份与健康归属筛选 | 按月份、现场、主平台或计划筛选健康结果 |
| 日期健康清单 | 每个日期集中展示当天纳管现场、计划数和问题摘要 |
| 现场 / 主平台层级 | 展开日期后先看现场，再看该现场下的主平台，不继续拆分子平台健康层级 |
| 全部执行记录 | 在抽屉中按模板、计划、来源和结果筛选逐次执行证据 |
...
3. 需要周期归档时点击“导出周/月报”。
4. 需要看整体趋势时点击“展开看板”。
5. 点击现场、主平台或具体计划的“查看”，打开分页执行记录，再展开某一次查看步骤、目标和判定依据。
...
2. Kafka 统一使用“消费组指标检测”：积压场景选择最大/总积压并与固定阈值比较；写入或消费停滞场景选择生产/消费 Offset 并与上次结果比较。
3. MQTT 设备心跳或采集消息使用 MQTT 主题活跃度检测；系统会保持后台订阅，计划只负责周期判定。
4. 数据只能从业务库准确获得时，使用数据库查询检查，并为巡检配置独立只读账号。
5. 检查目录积压时，区分 FTP 目录和服务器本地目录。
6. 检查服务器可用性时，区分磁盘、端口和服务状态。
7. 多台同类服务器尽量放在同一个步骤下配置多个子项，便于详情页按步骤聚合展示。
...
6. 保存步骤前先点击“测试并预览”；接口返回 JSON 时，系统会识别字段路径，点击字段即可添加为返回条件。
...
1. 后端会拒绝 INSERT、UPDATE、DELETE、DDL、多语句、`FOR UPDATE` 和数据库文件读取函数。
2. JDBC 连接强制设置为只读，并配置查询超时和最大返回行数。
3. “测试并预览”会展示字段名、前几行数据、返回行数和最终取值，便于保存前核对。
4. 正式巡检只记录取值、行数和调用摘要，不在记录中保存数据库密码。
...
| 现场融合管理 | 自动化巡检 |
| --- | --- |
| 维护现场、主平台、子平台、服务器和设备资产 | 为巡检计划提供现场/主平台健康归属，并从服务器资产树选择目标 |
...
1. 先在现场融合管理中维护清楚现场、主平台、子平台和服务器资产。
2. 新增巡检计划时选择所属现场；如果结果只代表某个主平台，则直接选择该主平台。
3. 子平台、服务器、接口和消息队列只作为检查目标和异常证据，不单独形成健康度层级。
4. 在巡检步骤里单独确认巡检执行账号和密码。
5. 服务器发生迁移或 IP 变更时，先更新现场融合管理，再检查相关巡检模板。
...
| 健康归属 | 选择现场级公共巡检，或选择现场下的一个主平台；不支持继续选择子平台 |
| 巡检模板 | 选择已经验证过的模板 |
| 任务编码 | 对应平台定时任务编码，用于排查调度 |
| 执行周期 | 页面可视化选择每日、每周、每月或间隔执行 |
| Cron 预览 | 系统自动生成，只读展示 |
| 状态 | 启用后按计划执行，暂停后不再触发 |
| 健康目标 | 当前计划每日健康度的目标百分比 |
...
3. 对关键业务建议设置固定每日巡检时间。
4. 所有定时计划都会进入每日健康；密集采样使用间隔执行，SSH递归目录等重型步骤不建议一分钟执行。
5. 选择“上次结果”时，还要按数据含义选择连续、按天或按小时累计；新周期首次采样建立基线并按正常计入健康度。
6. 修改计划后，下一次调度按最新配置执行。

### 17.1 每日健康汇总

每日汇总计划仍然绑定一个巡检模板，但原始采样不会直接堆满主列表。进入“巡检总览”，切换到“每日健康汇总”后，系统按照日期汇总以下信息：

每日健康汇总与逐次执行记录使用相同的筛选表单、归属日期块、表格行密度、异常行、结果摘要、统计格、操作列和分页规范。分页以完整日期分组为单位，同一天的多个计划不会被拆分；月度健康统计始终按完整月份计算。
...
当天只要出现过已确认异常，就保留“异常持续中”或“异常已恢复”标识，不会因为最后一次恢复正常而抹掉历史异常。逐次执行记录和每日汇总采样分开查询，现有周报继续以逐次执行记录为主。

### 17.2 累计数量的统计窗口

数值型步骤选择“上次结果”后，需要继续选择累计周期：

- **连续累计**：适合 Kafka Offset、累计消息数等跨日期持续增长的数据；计数器自身回退时可以重建基线。
- **每天重新累计**：适合今日过车、今日违法等每天从零开始的数据；跨日后的第一次取值建立当天基线，不与昨天末值相减。
- **每小时重新累计**：适合每小时重新统计或归零的业务数量；跨小时后的第一次取值建立新基线。

同一周期内数值下降不会被自动忽略。以今日过车为例，昨天末值为 20000000、今天凌晨值为 3000 时，系统识别日期已变化并建立正常基线；今天后续再以 3000 为上次值计算增长量。使用日期变量时，推荐用 `${todayStart}` 作为当天开始、用 `${endTime}` 作为本次有效结束时间。
...
| `前端工程/public/docs/auto-inspection/auto-inspection-manual.md` | 前端静态 Markdown 备份 |
| `前端工程/public/docs/auto-inspection/auto-inspection-manual.html` | 系统“打开完整文档”入口 |
...
升级到 `v4.2.0` 需要执行 `WDF100.0/sql/support_upgrade_20260901_auto_inspection_scope_health_v4_2_0.sql`。脚本新增现场/主平台归属快照和查询索引，并为历史自动执行记录补建缺失的每日汇总；不删除计划、执行记录、步骤结果或目标明细。升级后历史未归属计划会显示为“待归属”，编辑计划并选择现场或主平台后，后续结果立即按新归属统计。

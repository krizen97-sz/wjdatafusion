...
自动化巡检是平台面向日常运维巡查、系统健康检查和周期性报告归档建设的统一巡检能力。它把过去分散在人工登录、脚本执行、共享文档记录和临时截图里的检查动作，沉淀为“巡检工具、巡检步骤、巡检模板、巡检计划、巡检记录、巡检报告”这一套闭环流程。用户可以在页面中选择 Kafka、HTTP 接口调用测试、HTTP 健康、数据库只读查询、FTP 目录、服务器目录、磁盘、端口、服务状态、大数据服务器爆盘等巡检工具，按业务系统自由组合成模板，再通过手动执行或若依定时任务自动执行，最终形成可查询、可追溯、可导出的巡检结果。
...
- **数据来源**：本步骤从哪个接口、Kafka、FTP、服务器或数据库取数。
- **结果判断**：实际值满足什么条件才算正常。
...
3. 数据只能从业务库准确获得时，使用数据库查询检查，并为巡检配置独立只读账号。
4. 检查目录积压时，区分 FTP 目录和服务器本地目录。
5. 检查服务器可用性时，区分磁盘、端口和服务状态。
6. 多台同类服务器尽量放在同一个步骤下配置多个子项，便于详情页按步骤聚合展示。

...
class DataGovernanceKettleLiveTest
{
    ...
    @Test void originalCsvScriptFileRunsThroughOwnerGatewayAndDownloadsWholeArtifact() throws Exception
    {
        ...
        try
        {
            JsonNode capabilities = client.request("GET", "/capabilities", null); String xml = fixture(capabilities);
            ...
            service.upload(id, "input.csv", new ByteArrayInputStream("name\nalice\nbob\n".getBytes(StandardCharsets.UTF_8)), 73);
    ...
    private static String fixture(JsonNode capabilities)
    {
        ...
        for (int i = 0; i < types.length; i++)
        {
            ...
            if (i == 0) { put(step, "filename", "${WORK_DIR}/input.csv"); put(step, "encoding", "UTF-8"); put(step, "lazy_conversion", "N"); put(step, "fields/field/name", "name"); }
            if (i == 1)
            {

...
- 自动化巡检：以 Kafka、HTTP 接口调用测试、HTTP 健康、FTP、服务器目录、磁盘检测等基础工具配置巡检目标、巡检模板和巡检计划，定期执行后生成详情、Excel 和 Word 报告。
...
当前总版本为 `v3.13.0`。本版本在原有自动化巡检闭环内新增计划级高频监测、按日健康汇总、Kafka主题断流、Kafka消费停滞和MQTT持续监听能力；例行巡检、现场融合、文档管理和IP分配管控的既有流程保持兼容。

当前模块基线：

- 自动化巡检：高频健康监测 `v3.13.0`，支持例行/高频计划、每日健康、采样下钻、Kafka与MQTT活性工具，并完整保留v3.12.1看板、模板敏感凭据、长任务执行、标签树、数据库查询检查、测试预览和执行策略。
- 文档管理：普通用户共享候选范围修复 `v3.9.29`。
- 现场融合管理：设备机房、机柜和 U 位能力已在总版本 `v3.9.31` 整合。

...
当前总版本为 `v4.2.5`。本版本将自动化巡检驾驶舱改为六图表全看板，巡检总览继续保持 `v4.2.1` 的日期表格和层级结构。
...
- 自动化巡检：全图表驾驶舱 `v4.2.5`，使用七日趋势、现场/主平台双环构成、健康排行、计划完成度、问题矩形树图和执行时间轴统一表达驾驶舱数据；巡检总览保持 `v4.2.1` 表格结构。

...
当前总版本为 `v4.2.4`。本版本按使用反馈将巡检总览和驾驶舱恢复到 `v4.2.1` 的视觉结构，并对日期、现场、主平台层级和树形状态对齐做克制优化。
...
- 自动化巡检：界面回归与层级整理 `v4.2.4`，恢复 `v4.2.1` 的日期表格、摘要、双图表和树形清单，压缩日期展开留白并保持现场父级、主平台子级和状态列对齐。

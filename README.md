...
当前总版本为 `v3.10.0`。本版本在 `v3.9.31` 全模块统一基线上升级自动化巡检，新增数据库查询检查、测试数据预览、步骤复检/停止策略和可视化流程编排；文档管理、现场融合与 IP 分配管控继续保持上一版完整基线。

当前模块基线：

- 自动化巡检：数据源与可视化编排升级 `v3.10.0`，包含只读数据库检查、测试预览和执行策略。

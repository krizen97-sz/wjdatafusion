...
当前总版本为 `v3.13.3`。本版本进一步统一自动化巡检步骤配置的表单基线，并按业务权重压缩告警阈值等数值字段，使顶部控件和判定规则更加整齐易读。全部工具字段、测试、保存和执行逻辑保持兼容。

当前模块基线：

- 自动化巡检：步骤表单精细化优化 `v3.13.3`，统一顶部控件对齐与规则字段比例，并完整保留v3.13.2侧栏式配置工作区及既有巡检能力。

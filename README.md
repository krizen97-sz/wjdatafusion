...
当前总版本为 `v3.14.0`。本版本新增自动化巡检统一健康驾驶舱，并完成前端品牌、加载状态、错误页、标准表单和黑白主题基础治理；例行巡检与高频健康继续保留各自明细，同时在驾驶舱形成统一的当日健康结论。

当前模块基线：

- 自动化巡检：统一健康驾驶舱 `v3.14.0`，合并展示例行巡检、高频健康、计划状态与当日问题，并保留全部既有工具、模板、计划、记录和报告能力。

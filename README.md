...
当前总版本为 `v3.15.0`。本版本新增完全离线、阅读优先的轻量知识中心，提供知识维护、全文检索、不可覆盖版本和归档回收站，并通过文档ID安全联动现有文档管理附件。

当前模块基线：

- 平台通用：语义菜单图标库 `v3.14.3`，新增25个本地线性图标并统一主要业务菜单图标；默认人员头像更新保留在 `v3.14.2`。
- 知识中心：离线轻量知识库 `v3.15.0`，所有拥有专属维护权限的用户可直接创建和修改，每次保存留痕；附件实时复用文档管理权限与打开能力。

...
当前总版本为 `v3.12.0`。本版本将数据契约和结论先行方法内化为自动化巡检本地图表系统，重构趋势、结果构成、工具健康、异常TopN和月度日历热力图；其他模块继续保持 `v3.10.2` 统一主线基线。

当前模块基线：

- 自动化巡检：巡检看板本地图表系统 `v3.12.0`，完整保留模板敏感凭据、长任务执行、标签树、按日合并列表、数据库查询检查、测试预览和执行策略。

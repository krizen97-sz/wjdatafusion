...
当前总版本为 `v3.13.2`。本版本重新设计自动化巡检步骤配置弹窗，以左侧分区导航和右侧单一工作区替代纵向堆叠长表单，并保持全部工具字段、测试、保存和执行逻辑兼容。例行巡检、现场融合、文档管理和IP分配管控的既有流程保持不变。

当前模块基线：

- 自动化巡检：步骤配置工作区优化 `v3.13.2`，支持侧栏切换数据来源、结果判断和执行策略，复杂表单独立滚动，并完整保留v3.13.1工具契约及既有巡检能力。

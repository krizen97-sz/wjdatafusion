...
当前总版本为 `v4.0.4`。本版本建立“精准滑轨”全局交互语言，按页面密度和业务语义升级Tabs、Segmented、应用入口与执行反馈；保留全部既有路由、筛选、权限、接口和真实状态契约。
...
- 自动化巡检：巡检总览Tabs、高频结果筛选和驾驶舱计划筛选按精准滑轨精细化适配；`v4.0.3`日期分组分页、全月统计口径和今日计划筛选保持不变。

...
当前总版本为 `v3.16.4`。本版本将高频执行目标的本次、上次和变化数据改为纵向三行，并用结果区域悬浮提示替代独立详情按钮；v3.16.3 的同日多计划导航与状态解释保持不变。
...
- 自动化巡检：高频判定数据与调用结果阅读优化 `v3.16.4`，三项判定数据纵向排列、单项标签与数值横向展示，调用结果悬浮即可查看完整信息；v3.16.3同日多计划导航保持不变。

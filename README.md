...
当前总版本为 `v4.2.1`。本版本补齐自动化巡检从 `v4.1.1` 到最新模型的累计升级与旧数据归属迁移能力，不改变 `v4.2.0` 运行时业务逻辑。
...
- 自动化巡检：累计升级与旧数据归属迁移 `v4.2.1`，支持预览、高置信度自动推导、手工覆盖、冲突保护、历史快照回填和每日健康补建；`v4.2.0` 现场/主平台健康逻辑保持不变。

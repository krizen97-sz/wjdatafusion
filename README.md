...
当前总版本为 `v4.2.2`。本版本优化自动化巡检驾驶舱与巡检总览的现场/主平台健康展示，取消日期展开缩进，并补齐异常、关注和待归属数据的直接下钻入口。
...
- 自动化巡检：健康层级与异常直达优化 `v4.2.2`，驾驶舱树形状态对齐，总览按日期全宽展示现场和主平台健康，异常、关注、缺失和待归属数量可直接进入对应明细或配置。

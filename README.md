...
当前总版本为 `v3.16.2`。本版本将高频当日结果改为分页、单项折叠的执行列表，统一计划文案和状态展示，并通过独立脚本修复旧版首次基线被记为未执行而拉低健康度的问题；v3.16.1 文档 JWT 签名修复保持不变。
...
- 自动化巡检：高频每日结果折叠与历史健康度修复 `v3.16.2`，固定阈值和上次结果比较继续共用取数工具；日期行一次点击进入分页执行记录，每次执行默认收起且只展开一条，旧版首次基线可通过独立脚本安全修正并重算健康度。

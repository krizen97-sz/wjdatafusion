...
当前总版本为 `v3.16.0`。本版本重构自动化巡检的数值判定与高频每日结果查看链路，首次采样正常计入健康度，固定阈值和上次结果比较统一配置，并保留v3.15.2离线图标库及v3.15.1知识中心布局能力。
...
- 自动化巡检：数值判定与每日结果直达 `v3.16.0`，固定阈值和上次结果比较共用取数工具，首次基线按正常计分，并可一次点击查看当日采样、步骤、目标和完整判定依据。

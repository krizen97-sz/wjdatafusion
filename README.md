...
当前总版本为 `v3.12.1`。本版本撤销上一版巡检看板图表重构，恢复原有统计卡、日期网格、趋势图、环形结果图、工具健康度和异常分布显示；其他模块继续保持 `v3.10.2` 统一主线基线。

当前模块基线：

- 自动化巡检：巡检看板显示回退 `v3.12.1`，完整保留v3.11.2模板敏感凭据、长任务执行、标签树、按日合并列表、数据库查询检查、测试预览和执行策略。

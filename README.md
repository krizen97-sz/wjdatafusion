...
当前总版本为 `v3.15.2`。本版本全量接入547个Keyline离线线性图标，保留既有115个平台图标与菜单映射，并使用Element Plus原生组件重构图标来源、分类和多语言搜索；v3.15.1知识中心布局及v3.15.0知识、版本和文档联动能力保持不变。

当前模块基线：

- 平台通用：离线扩展图标库 `v3.15.2`，现有115个平台图标与547个`keyline-*`扩展图标分层共存，后续页面禁止为单个场景手工绘制新SVG；既有菜单语义映射保持不变。
- 知识中心：原生组件布局重构 `v3.15.1`，采用两栏主从结构、原生搜索/表格/抽屉/折叠面板并支持桌面宽度自适应；v3.15.0 的离线知识、版本留痕和文档权限联动保持不变。
- 自动化巡检：统一健康驾驶舱 `v3.14.1`，驾驶舱、总览、配置与其他模块可以稳定连续切换，并保留例行巡检、高频健康、计划状态和当日问题的统一展示。

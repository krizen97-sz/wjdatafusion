...
当前总版本为 `v3.16.5`。本版本将例行巡检记录与高频每日健康改为Element Plus默认Tabs导航，分离内容切换和页面命令，并支持通过URL恢复当前视图；v3.16.4目标明细优化保持不变。
...
- 自动化巡检：巡检总览原生Tabs导航重构 `v3.16.5`，例行巡检与高频每日健康使用默认Tabs切换并持久化URL状态；v3.16.4纵向判定数据和悬浮调用结果保持不变。

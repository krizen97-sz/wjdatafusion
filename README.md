...
当前总版本为 `v3.11.2`。本版本统一自动化巡检模板与目标编辑页的密码、Secret和提权密码回显逻辑，所有已保存敏感凭据均通过权限接口由小眼睛按需显示；其他模块继续保持 `v3.10.2` 统一主线基线。

当前模块基线：

- 自动化巡检：模板敏感凭据统一回显 `v3.11.2`，完整保留长任务执行、标签树、按日合并列表、数据库查询检查、测试预览和执行策略。

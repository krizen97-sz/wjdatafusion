...
当前总版本为 `v3.13.1`。本版本补充自动化巡检工具的前后端配置契约，确保Kafka主题断流、Kafka消费停滞和MQTT持续监听分别进入对应的数据来源表单；未知工具不再错误回退为服务器配置。例行巡检、现场融合、文档管理和IP分配管控的既有流程保持兼容。

当前模块基线：

- 自动化巡检：工具配置契约修复 `v3.13.1`，支持例行/高频计划、每日健康、采样下钻、Kafka与MQTT活性工具，并阻止前后端版本不一致时套用错误目标表单。

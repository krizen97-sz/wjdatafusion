...
当前总版本为 `v3.10.1`。本版本修复自动化巡检数据库查询工具在 MySQL 客户端字符集配置不一致时出现的中文乱码，并为初始化、整合部署和专项升级脚本增加 UTF-8 字符集保护；`v3.10.0` 的数据库查询检查、测试数据预览、步骤复检/停止策略和可视化流程编排能力保持不变。

当前模块基线：

- 自动化巡检：数据库查询工具字符集修复 `v3.10.1`，完整保留 `v3.10.0` 的只读数据库检查、测试预览和执行策略。

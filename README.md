...
当前总版本为 `v4.8.6`。本版本恢复巡检展开详情原有的底色、步骤序号与判定数据层次；保留 v4.8.5 的记录列表统一及文字遮挡修复，不改变详细字段、判定逻辑和操作流程，无数据库升级。
...
- 自动化巡检：展开详情层次恢复 `v4.8.6`，沿用原来的步骤序号、分组和判定数据底色；保留统一记录列表及长名称可读性修复，六图表驾驶舱与日期健康表格保持不变。

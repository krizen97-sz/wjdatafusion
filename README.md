...
当前总版本为 `v3.10.3`。本版本重构自动化巡检总览和配置工作区：巡检记录按日期归集，首页补充紧凑趋势与结果图表，配置页统一为单一的模板编排/执行计划切换入口；其他模块继续保持 `v3.10.2` 统一主线基线。

当前模块基线：

- 自动化巡检：总览与配置交互优化 `v3.10.3`，完整保留数据库查询检查、测试预览、执行策略和字符集修复。

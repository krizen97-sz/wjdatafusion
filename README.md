...
当前总版本为 `v3.11.0`。本版本面向电脑端工作台统一自动化巡检记录、模板和计划的组织方式：记录列表按日期合并首列，模板和计划支持持久化标签，并通过Element Plus树选择按标签目录完成筛选和绑定；其他模块继续保持 `v3.10.2` 统一主线基线。

当前模块基线：

- 自动化巡检：标签树与按日合并列表 `v3.11.0`，完整保留数据库查询检查、测试预览、执行策略和字符集修复。

...
当前总版本为 `v4.8.5`。本版本统一巡检执行记录列表与步骤、检查子项详情，修复长模板名、标签和子项内容挤压遮挡；保留分页筛选、导出及原健康度计算逻辑，无数据库升级。
...
- 自动化巡检：执行记录与子项详情统一展示 `v4.8.5`，全部记录和平台明细复用列表、状态及步骤结果组件；名称和标签分行，长数据可读，六图表驾驶舱与日期健康表格保持不变。

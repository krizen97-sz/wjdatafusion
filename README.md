...
当前总版本为 `v4.1.1`。本版本修复自动化巡检累计周期配置的Vue setup初始化顺序错误，恢复巡检总览和配置路由；v4.1.0统计窗口与统一计划模型保持不变。
...
- 自动化巡检：累计周期初始化热修 `v4.1.1`，巡检总览与配置页面恢复正常；v4.1.0连续、按天、按小时统计窗口及逐次记录/每日汇总模型保持不变。

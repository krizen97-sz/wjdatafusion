...
当前总版本为 `v3.11.1`。本版本修复自动化巡检模板/计划手动执行被全局10秒请求时限提前中断的问题，并恢复数据库查询步骤的数据库类型、只读SQL和密码明文查看；其他模块继续保持 `v3.10.2` 统一主线基线。

当前模块基线：

- 自动化巡检：模板执行与数据库配置回显修复 `v3.11.1`，完整保留标签树、按日合并列表、数据库查询检查、测试预览和执行策略。

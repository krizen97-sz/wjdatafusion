...
public final class KettleWorker {
  ...
  static void execute(TransMeta meta,String target,int limit)throws Exception {
    if(!target.isEmpty())previewGraph(meta,target);Trans trans=new Trans(meta);trans.setLogLevel(LogLevel.ERROR);AtomicInteger logCount=new AtomicInteger();KettleLogStore.getAppender().addLoggingEventListener(logEvent->{if(logCount.incrementAndGet()<=500)event("log",Map.of("level",String.valueOf(logEvent.getLevel()),"message",safe(String.valueOf(logEvent.getMessage()))));});event("state",Map.of("state","PREPARING","nodes",nodes(meta)));

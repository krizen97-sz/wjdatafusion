...
此模块把当前安全图和显式 JSON 输入发布为不可变版本，再将 Cron 计划绑定到该版本。输入模式为 `FIXED_JSON_BATCH`：每次运行读取发布时保存的同一份 JSON 对象或数组。v4.7.0支持可选FTP批次交付；持续数据源、自动Kafka取批调度和海康完整业务适配尚未接入。显式Kafka取批使用独立[Kafka批次工作台](DATA_GOVERNANCE_KAFKA_BATCH.md)。

## 执行与版本边界

- 发布时沿用安全执行白名单、2 至 32 节点、96 条连接、无环、一个样本输入和一个结果观察节点的校验；禁止任意脚本、外部系统、子流程及连接服务。
...
可选`deliveryConnectionId`随计划绑定并冻结目标指纹。整批成功、清理和完整产物确认后才提交FTP，保持执行占用至交付完成；EMPTY明确跳过，失败或重启只核查已有运行和交付，不重放未知效果。API创建/编辑请求支持该可选字段，计划摘要包含交付状态与交付记录ID；详见[完整产物与批次交付](DATA_GOVERNANCE_DELIVERY.md)。

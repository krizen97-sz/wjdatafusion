...
版本和计划保存于 `${data-governance.storage-dir}/schedules/`；目录 700、文件 600、随机临时文件 + fsync + 原子替换，独立文件锁保证单实例。版本创建后不可覆盖。拒绝无效 UUID、符号链接、超过 4 MiB 的文件；单用户最多 500 个版本和 100 个计划，读取每类最多 10000 条记录。

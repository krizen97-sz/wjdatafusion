...
| 紧凑互斥筛选 | `el-segmented.motion-segmented` + option 图标 | 复用原生滑动选中面，只用单层轻阴影，不加选中边框或下划线；文字统一使用 `--el-font-size-base`，选中文字使用标题色，普通图标使用主色；共享标签对系统字体做2px向上光学校正；局部决定等宽、换行或横向滚动 |

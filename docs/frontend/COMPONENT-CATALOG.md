...
| 图标文字标签 | `.motion-control-label` + `SvgIcon` | Tabs / Segmented 的 label/default slot | Segmented 文字使用 `--el-font-size-base` 并在共享样式维护2px光学校正；页面不缩小字号或重复偏移；不写内联 SVG 或 Emoji |
